...
def _bootstrap():
    """
    Handles three Linux scenarios in order:
      1. Deps already available — do nothing.
      2. Running inside a venv we created — install missing packages.
      3. Running on a system Python (externally-managed, e.g. Mint/Ubuntu 23+)
         — create a local .venv next to the script, install there, re-exec.
    Includes generous timeouts + retries to survive slow networks.
    """
    ...
    # Shared pip install flags — generous timeout + retries for slow connections
    PIP_FLAGS = [
        "--quiet",
        "--timeout", "120",
        "--retries", "5",
    ...
    def _pip_install(python_exe, packages, extra_flags=None):
        cmd = [python_exe, "-m", "pip", "install", *PIP_FLAGS, *(extra_flags or []), *packages]
        return subprocess.call(cmd)

    # ── Strategy 1: plain pip (works in most venvs / Windows) ──
    rc = _pip_install(sys.executable, missing)
    if rc == 0:
        print("✅  Installed!  Restarting…\n")
        os.execv(sys.executable, [sys.executable] + sys.argv)

    # ── Strategy 2: --break-system-packages (some managed envs allow it) ──
    rc = _pip_install(sys.executable, missing, ["--break-system-packages"])
    if rc == 0:
        print("✅  Installed!  Restarting…\n")
        os.execv(sys.executable, [sys.executable] + sys.argv)

    # ── Strategy 3: create a local venv and install there ──
    script_dir = Path(sys.argv[0]).resolve().parent
    venv_dir   = script_dir / ".myscreen_venv"

    print(f"📁  Creating local virtual environment at:\n    {venv_dir}\n")
    rc = subprocess.call([sys.executable, "-m", "venv", str(venv_dir)])
    if rc != 0:
        # python3-venv might not be installed on the system
        print("⚠️   venv creation failed — trying to install python3-venv via apt…")
        subprocess.call(["sudo", "apt-get", "install", "-y", "python3-venv", "python3-full"],
    ...
    # Resolve the venv python binary (Linux vs Windows path differ)
    venv_python = (
        venv_dir / "bin"    / "python"
    ...
    rc = _pip_install(str(venv_python), missing)
    if rc != 0:
        _fatal(missing)

    print("\n✅  All set!  Re-launching with venv Python…\n")
    os.execv(str(venv_python), [str(venv_python)] + sys.argv)


def _fatal(missing):
    import sys
    print("\n❌  Automatic install failed on all strategies.")
    print("    Run ONE of these manually, then try again:\n")
    pkgs = " ".join(missing)
    print(f"    # Option A — venv (recommended on Linux Mint):")
    print(f"    python3 -m venv .myscreen_venv")
    print(f"    source .myscreen_venv/bin/activate")
    print(f"    pip install {pkgs}\n")
    print(f"    # Option B — force system install (use at own risk):")
    print(f"    pip3 install --break-system-packages {pkgs}\n")
    sys.exit(1)
