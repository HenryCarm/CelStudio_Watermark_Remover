...
class _BaseCanvas(QWidget):
    ...
    def wheelEvent(self, e) -> None:
        ...
        self.scale = max(0.05, min(16.0, self.scale * factor))
        self._recenter()
        self.update()
...
class Canvas(_BaseCanvas):
    ...
    def _display_img(self)  -> QImage | None: return self._qi
    ...
    def save_result(self) -> Path | None:
        ...
        return out if cv2.imwrite(str(out), self.cv_cur) else None
...
class VideoTab(QWidget):
    ...
    def _toggle_play(self) -> None:
        if self._playing:
            ...
            self._pt.start(max(1, int(1000 / self._fps)))
...
class GalleryView(QWidget):
    ...
    def refresh(self) -> None:
        ...
        if   idx == 0: imgs.sort(key=lambda p: p.stat().st_mtime, reverse=True)
        elif idx == 1: imgs.sort(key=lambda p: p.stat().st_mtime)
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        logo = QLabel("🪄  MyScreen WR")
        logo.setStyleSheet(
            f"font-size:13px;font-weight:700;color:{PURPLE_LIGHT};"
        ...
        vb.setStyleSheet(
            f"color:{TEXT_FAINT};font-size:9px;"
            f"padding:1px 5px;background:{BG_CARD};"
        ...
        self._undo_btn.setObjectName("icon_btn")
        ...
        self._undo_lbl.setStyleSheet(
            f"color:{TEXT_FAINT};font-size:9px;min-width:20px;")
        tl.addWidget(self._undo_lbl)

        self._redo_btn = QPushButton("↪")
        self._redo_btn.setObjectName("icon_btn")
        ...
        # Divider
        d1 = QFrame(); d1.setFrameShape(QFrame.Shape.VLine)
        d1.setStyleSheet(
            f"color:{BORDER_LIT};max-height:24px;margin:0 4px;")
        tl.addWidget(d1)

        # Presets toggle
        self._presets_btn = QPushButton("⚡ Presets")
        self._presets_btn.setObjectName("ghost")
        self._presets_btn.setFixedHeight(26)
        self._presets_btn.setCheckable(True)
        self._presets_btn.setToolTip("Toggle presets panel  [Ctrl+Shift+P]")
        self._presets_btn.clicked.connect(self._toggle_presets)
        tl.addWidget(self._presets_btn)
        ...
        self._bb = ToolBtn("🖌", "Brush  [B]")
        self._eb = ToolBtn("🧽", "Eraser  [X]")
        self._rb = ToolBtn("▭",  "Rectangle  [R]")
        self._cb = ToolBtn("⭕", "Ellipse  [E]")
        self._bb.setChecked(True)
        self._tg = QButtonGroup(self); self._tg.setExclusive(True)
        for btn in [self._bb, self._eb, self._rb, self._cb]:
            self._tg.addButton(btn); sl.addWidget(btn)
        self._bb.clicked.connect(
            lambda: self._set_tool(_BaseCanvas.TOOL_BRUSH))
        self._eb.clicked.connect(
            lambda: self._set_tool(_BaseCanvas.TOOL_ERASER))
        self._rb.clicked.connect(
            lambda: self._set_tool(_BaseCanvas.TOOL_SQUARE))
        self._cb.clicked.connect(
            lambda: self._set_tool(_BaseCanvas.TOOL_CIRCLE))

        # Divider
        d2 = QFrame(); d2.setFrameShape(QFrame.Shape.HLine)
        d2.setStyleSheet(f"color:{BORDER};")
        sl.addSpacing(4); sl.addWidget(d2); sl.addSpacing(4)

        # Size slider (compact, vertical)
        self._sz_lbl = QLabel("25")
        self._sz_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._sz_lbl.setStyleSheet(
            f"color:{PURPLE_LIGHT};font-size:10px;font-weight:600;")
        sl.addWidget(self._sz_lbl)
        self._sz_sl = QSlider(Qt.Orientation.Vertical)
        self._sz_sl.setRange(2, 100)
        self._sz_sl.setValue(self._s.get("brush_size", 25))
        self._sz_sl.setFixedHeight(90)
        self._sz_sl.valueChanged.connect(self._set_brush_size)
        sl.addWidget(self._sz_sl, alignment=Qt.AlignmentFlag.AlignHCenter)

        sl.addStretch()

        # Zoom controls
        d3 = QFrame(); d3.setFrameShape(QFrame.Shape.HLine)
        d3.setStyleSheet(f"color:{BORDER};"); sl.addWidget(d3)
        sl.addSpacing(3)
        zs = (f"QToolButton{{background:transparent;color:{TEXT_DIM};"
              f"border:1px solid {BORDER};border-radius:5px;"
              f"font-size:13px;font-weight:600;}}"
              f"QToolButton:hover{{color:{PURPLE_LIGHT};"
              f"background:{BG_RAISED};border-color:{PURPLE};}}")
        for txt, fn in [("+", lambda: self._zoom(1.2)),
                         ("−", lambda: self._zoom(0.83)),
                         ("⊙", self._zoom_fit)]:
            zb = QToolButton(); zb.setText(txt)
            zb.setFixedSize(34, 22); zb.setStyleSheet(zs)
            zb.clicked.connect(fn); sl.addWidget(zb, alignment=Qt.AlignmentFlag.AlignHCenter)
        el.addWidget(sb)
        ...
        # Bottom action bar — intentionally minimal
        bot = QFrame(); bot.setFixedHeight(48)
        bot.setStyleSheet(
            f"background:{BG_BASE};border-top:1px solid {BORDER};")
        bl = QHBoxLayout(bot)
        ...
        # Overflow menu for clear/reset (keeps toolbar clean)
        self._more_btn = QPushButton("· · ·")
        self._more_btn.setObjectName("ghost")
        self._more_btn.setFixedSize(36, 28)
        ...
        # Mode selector + help bubble
        mh_row = QHBoxLayout(); mh_row.setSpacing(3)
        mhl = QLabel("Mode:")
        mhl.setStyleSheet(f"color:{TEXT_DIM};font-size:10px;")
        mh2 = HelpBubble(
            "⚡ Quick  —  Fast TELEA inpaint.\n"
            "             Best for small logos.\n\n"
            "🧠 Smart  —  Ring-by-ring fill.\n"
            "             No blur on large covered areas.\n"
            "             Use for most watermarks.\n\n"
            "🔬 Precision  —  Exemplar patch-match\n"
            "             (like Photoshop Content-Aware Fill).\n"
            "             Best quality, slower.\n"
            "             Great for complex backgrounds.")
        self._hbs.append(mh2)
        mh_row.addWidget(mhl); mh_row.addWidget(mh2)
        bl.addLayout(mh_row)
        ...
        self._lvl_box.setFixedWidth(120)
        self._lvl_box.setFixedHeight(28)
        bl.addWidget(self._lvl_box)

        # Primary CTA
        self._remove_btn = QPushButton("✨  Remove Watermark")
        ...
        self._set_status("  Ready — open an image or drop one onto the canvas")
    ...
    def _adapt_layout(self) -> None:
        """Compact mode when window is narrow."""
        narrow = self.width() < MOBILE_WIDTH_THRESHOLD
        # In narrow mode: hide sidebar labels, compress bottom bar
        self._sz_lbl.setVisible(not narrow)
        if narrow:
            self._sz_sl.setFixedHeight(70)
        else:
            self._sz_sl.setFixedHeight(90)
    ...
    def _set_busy(self, busy: bool) -> None:
        for w in [self._open_btn, self._more_btn, self._remove_btn,
    ...
    def _zoom(self, f: float) -> None:
        self.canvas.scale = max(0.05, min(16.0, self.canvas.scale * f))
        self.canvas._recenter(); self.canvas.update()
    ...
    def _save_image(self) -> None:
        ...
        if path:
            ...
            self._set_status("  ❌  Save failed — check folder permissions")
